...
- Release/deploy SHA matches an already-reviewed `main` SHA, and full test suite is green + build succeeds
- No open 🔴 CRITICAL issues, and all 🟡 IMPORTANT issues from the release SHA's reviews have been resolved OR explicitly risk-accepted (comment on issue with rationale)
