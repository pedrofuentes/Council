...
1. **Structural sanitisation** — `sanitizePromptField` / `sanitizePromptBlock` / `sanitizeFenced` / `escapeFenceContent` in `src/core/prompt-sanitize.ts`. Strips C0/C1 controls, bidi/zero-width characters, collapses Unicode line separators (NEL / U+2028 / U+2029), defangs `[N]` section markers, caps length. Field variant collapses newlines (for system-prompt fields); block variant preserves them (for fenced bodies).
