...
### ADR-031: Deterministic phase-prompt templates for structured debates; LLM-driven moderator deferred
**Date**: 2026-07-02
**Status**: Accepted
**Context**: ROADMAP §2.2 (Structured Debate Engine) needed a fixed 4-phase choreography — opening → cross-examination → rebuttal → synthesis — where the cross-examination and rebuttal prompts must reference specific claims from other experts' prior turns (`core/moderator/phase-prompts.ts`). The original vision for this seam (ROADMAP §2.3, "Pluggable Moderator Strategies") was a moderator-LLM that reads the opening statements and generates per-pair, *targeted* cross-examination questions. That requires a second model call per turn plus a moderator-orchestrator interface (e.g. a `ModeratorStrategy.generateCrossExamPrompt(...)` hook) that did not exist, and it would make structured-debate phase prompts depend on live model output — breaking the fully offline, deterministic `MockEngine` coverage the test suite relies on (`structured-debate.test.ts`: "Cross-examination questions are deterministic in this PR... An LLM-driven moderator that generates targeted questions ships in ROADMAP §2.3"). §2.3 shipped instead as a `ModeratorStrategy` interface (`planRound`/`shouldContinue`) with three built-in **rule-based** strategies — `round-robin`, `devils-advocate`, `consensus-check` (`core/moderator/strategies.ts`) — wired only into freeform mode (`Debate.#runFreeform()`). No strategy in that set is LLM-driven, so a moderator that dynamically chooses phases, targets, or strategy left the implemented scope entirely; Sentinel (PR#106 cycle 2 finding 7) flagged that this trade-off — deterministic phase templates now vs. a smarter moderator later — was undocumented.
**Decision**: Structured mode (`DebateConfig.mode === "structured"`) always uses the deterministic, pure phase-prompt builders in `core/moderator/phase-prompts.ts` (`buildOpeningPrompt`, `buildCrossExamPrompt`, `buildRebuttalPrompt`, `buildSynthesisPrompt`). These quote other experts' prior-turn content verbatim inside sanitized `<from_expert>` fences (`sanitizeFenced` + `safeAttrName` — see that file's header) and instruct the *answering* expert to "identify the SPECIFIC claim ... you find weakest" themselves; the template supplies the quoting and framing, not a moderator's judgment of what to ask. `DebateConfig.strategy` (the pluggable `ModeratorStrategy`) is ignored entirely in structured mode — the fixed phase sequence always wins, with cross-examination skipped for single-expert panels (3 phases: opening/rebuttal/synthesis) since there is no one to cross-examine. An LLM-based moderator that dynamically generates cross-exam questions, or otherwise chooses phase content, remains explicitly deferred to a future iteration; `phase-prompts.ts`'s header documents the intended seam ("The §2.3 PR will swap these templates for a `ModeratorStrategy.generateCrossExamPrompt(...)` call").
**Alternatives considered**:
- **LLM moderator dynamically generates targeted per-pair cross-exam questions now.** Deferred, not rejected outright: it is the intended long-term design, but it needs its own premium-request budget, a `generateCrossExamPrompt`-style hook added to `ModeratorStrategy`, and a fresh prompt-injection review for moderator-authored (not just expert-authored) content. Building it inside the §2.2 structured-debate PR would have expanded scope and made phase prompts non-deterministic, breaking offline `MockEngine` coverage.
- **Retrofit `ModeratorStrategy.planRound()` to also drive structured mode.** Rejected: `planRound` returns one assignment list per uniform round; it has no concept of 4 fixed phases or of threading opening/cross-exam turns forward into later rebuttal/synthesis prompts. Bending the freeform-oriented interface to fit would complicate a currently simple, pure abstraction for a shape it was not designed to hold.
- **Run all 4 phases unconditionally, even for single-expert panels.** Rejected: cross-examination has no target with only one expert on the panel; `debate.ts` skips it (3-phase sequence) rather than emitting a meaningless self-directed round.
**Consequences**:
- ✅ Structured debates are fully deterministic and offline-testable: every `phase-prompts.ts` function is pure (no I/O, no engine calls, no clock reads), so `structured-debate.test.ts` runs entirely against `MockEngine` with reproducible prompt strings.
- ✅ No added per-turn model-call cost or latency for question generation — cross-examination framing is free text-templating, not a second premium request.
- ✅ The prompt-injection surface stays small and testable: all quoted expert content flows through the same fixed set of template functions and sanitizers (`sanitizeFenced`, `safeAttrName`), rather than also having to defend against open-ended, unpredictable moderator-LLM output.
- ⚠️ Cross-examination questions are templated, not targeted: every expert receives the same "find the other side's weakest claim" instruction rather than a question a moderator actually curated from the specific disagreement. Quality depends on the answering expert's own analysis of the quoted content.
- ⚠️ `--strategy` is a silent no-op in structured mode (`DebateConfig.strategy` is ignored there by design, per `debate.ts`'s module header) — a user combining `--mode structured --strategy devils-advocate` gets no error and no devil's-advocate behavior.
- 📝 **When to swap**: revisit once (a) the cost/latency budget can absorb an extra premium request per cross-exam turn, and (b) a moderator-orchestrator seam (`ModeratorStrategy.generateCrossExamPrompt(...)` or equivalent) exists to own that call and its own prompt-injection review. Until then, deterministic templates remain the right default for testability, cost, and offline (`MockEngine`) coverage.
