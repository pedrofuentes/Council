/**
 * Regression tests: `buildReviewCommand` threads `config.qualityGate` into
 ...
 * `review.ts:~260`, `DebateConfig.qualityGate` becomes `undefined`, the
 ...
 * `config.yaml`, run review with `--diff-file <path> --max-rounds 1`, supply
 * a scripted engine whose first expert response contains a forbidden phrase
 * (Layer-1 failure), then assert the plain renderer's quality-gate notice
 * (`quality gate:`) appears in stdout.
 */
...
// ---------------------------------------------------------------------------
// Scripted engine — identical to the one in convene-qualitygate-threading.test.ts
// (kept local to avoid cross-test-file coupling).
// ---------------------------------------------------------------------------
...
// A small, deterministic unified diff that gives the review experts something
...
describe("review — config.qualityGate threading (regression #1514)", () => {
  let testHome: string;
  let diffFile: string;
  ...
  beforeEach(async () => {
    setQuiet(false);
    testHome = await fs.mkdtemp(path.join(os.tmpdir(), "council-qg-review-"));
    originalHome = process.env["COUNCIL_HOME"];
    process.env["COUNCIL_HOME"] = testHome;
    await copyTemplateDb(path.join(testHome, "council.db"));
    // Write the diff file into testHome (never /tmp).
    diffFile = path.join(testHome, "changes.diff");
    await fs.writeFile(diffFile, SAMPLE_DIFF, "utf-8");
