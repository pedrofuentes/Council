 ...
 *   4. `CI`                     — unset / empty / set
 *   5. `COUNCIL_NO_TUI`         — unset / empty / set (persistent opt-out)
 *   6. `COUNCIL_TUI`            — unset / empty / "1" (legacy opt-in force)
 *
...
function expectedLaunch(inputs: LaunchInputs): boolean {
  ...
  if (inputs.isTTY !== true) return false; // non-TTY falls back to the CLI
...
describe("shouldLaunchTui", () => {
  describe("headline behaviors (the 9.10 default-flip)", () => {
    it("defaults ON for bare `council` on a TTY (COUNCIL_TUI unset)", () => {
      expect(shouldLaunchTui(["node", "council"], { stdout: TTY, env: {} })).toBe(true);
    });

    it("ignores leading global flags when detecting a bare invocation", () => {
      expect(shouldLaunchTui(["node", "council", "-q"], { stdout: TTY, env: {} })).toBe(true);
    });

    it("returns false when `--no-tui` is present (it is a flag, not a subcommand)", () => {
      expect(shouldLaunchTui(["node", "council", "--no-tui"], { stdout: TTY, env: {} })).toBe(
        false,
      );
    });
    ...
    it("returns false for any non-flag subcommand", () => {
      for (const sub of ["convene", "doctor", "chat", "panel"]) {
        expect(shouldLaunchTui(["node", "council", sub], { stdout: TTY, env: {} })).toBe(false);
      }
    });

    it("returns false on a non-TTY / piped stdout", () => {
      expect(shouldLaunchTui(["node", "council"], { stdout: NON_TTY, env: {} })).toBe(false);
    });

    it("returns false when CI is set and non-empty", () => {
      expect(shouldLaunchTui(["node", "council"], { stdout: TTY, env: { CI: "true" } })).toBe(
        false,
      );
    });

    it("still defaults ON when CI is present but empty", () => {
      expect(shouldLaunchTui(["node", "council"], { stdout: TTY, env: { CI: "" } })).toBe(true);
    });

    it("returns false when COUNCIL_NO_TUI is set and non-empty", () => {
      expect(
        shouldLaunchTui(["node", "council"], { stdout: TTY, env: { COUNCIL_NO_TUI: "1" } }),
      ).toBe(false);
    });

    it("still defaults ON when COUNCIL_NO_TUI is present but empty", () => {
      expect(
        shouldLaunchTui(["node", "council"], { stdout: TTY, env: { COUNCIL_NO_TUI: "" } }),
      ).toBe(true);
    });

    it("honors COUNCIL_TUI=1 as an explicit force on a bare TTY", () => {
      expect(shouldLaunchTui(["node", "council"], { stdout: TTY, env: { COUNCIL_TUI: "1" } })).toBe(
        true,
      );
    });
  ...
  describe("exhaustive cartesian sweep over every input dimension", () => {
    ...
    it("matches the specification oracle for all input combinations", () => {
            ...
            for (const ci of cis) {
              for (const councilNoTui of noTuiEnvs) {
                for (const councilTui of tuiEnvs) {
                  ...
                  expect(
                    actual,
                    `argv=${JSON.stringify(argv)} isTTY=${isTTY} CI=${String(ci)} ` +
                      `COUNCIL_NO_TUI=${String(councilNoTui)} COUNCIL_TUI=${String(councilTui)}`,
                  ).toBe(expected);
                  combos += 1;
                }
              }
      ...
      // 3 subcommands × 2 noTui × 2 tty × 3 CI × 3 NO_TUI × 3 TUI = 324
