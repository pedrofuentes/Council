...
vi.mock("../../../src/memory/persister.js", () => {
  class MockDebatePersister {
    // A plain readonly field (NOT a `get debateId()` accessor) so the mock is
    // lint-clean under @typescript-eslint/class-literal-property-style.
    readonly debateId = "mock-debate";

    constructor(deps: DebatePersisterDeps) {
      capturedDeps.push(deps);
    }

    // Matches the real `DebatePersister.persist(source, prompt)` signature and
    // return type. The wiring test only needs the captured logger, so the real
    // debate source is deliberately left un-iterated: `persist` returns a
    // manual async-iterable that reports completion on the first pull, yielding
    // nothing. It is intentionally NOT an `async *` generator — a generator
    // that never yields trips the `require-yield` lint rule (the cause of the
    // #1953 revert).
    persist(_source: AsyncIterable<DebateEvent>, _prompt: string): AsyncIterable<DebateEvent> {
      return {
