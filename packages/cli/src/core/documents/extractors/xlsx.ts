 ...
 * XLSX extractor (T8).
 *
 * Uses the dependency-light `xlsx-reader` (yauzl + fast-xml-parser) to
 * parse `.xlsx` workbook buffers and emit one Markdown table per sheet,
 * prefixed by a `## SheetName` header. The first row of each sheet is
 * treated as the table header. Sheet names are also surfaced via
 * `metadata.sheetNames` so higher layers can present them without
 * re-parsing.
 *
 * `.xls` (BIFF8 binary) is intentionally NOT supported; we still
 * register the extension so the user gets a precise, actionable
...
import { readXlsxSheets, XlsxEncryptedError, type XlsxSheet } from "./xlsx-reader.js";
...
// preflight over the central directory before parsing so we can reject
// decompression-bomb archives (huge entry counts, pathological
// compression ratios, or oversized uncompressed totals) up front. After
// the preflight succeeds we still cap CPU/heap totals observed during
// traversal — the reader can synthesize cells from SharedStrings /
// styles / formulas in ways that aren't visible from the ZIP central
// directory alone, so the post-load caps remain a necessary second line
// of defense.
...
const OVERSIZE_SUGGESTION = "Reduce the number of sheets/rows or split into smaller files.";
...
async function preflightZip(buffer: Buffer, filename: string): Promise<void> {
  ...
  try {
    ...
    // compound document), random binary, or text. Return so that the
    // reader can take over and surface the correct format-specific error
    // (the legacy `.xls` branch produces an actionable "re-save as
    // .xlsx" suggestion). This is the ONLY yauzl failure mode we
    ...
    // offset trips yauzl — and must be surfaced rather than handed off
    // to a more lenient parser.
...
const xlsxExtractor: ContentExtractor = async (
  ...
  // OLE-prefixed ZIP polyglots evade an OLE deny-list because more
  // lenient parsers tolerate the shifted central directory. An
  ...
  // when the buffer is genuinely not a ZIP (e.g., real BIFF8 `.xls` or
  // an encrypted OLE package) and there is no bomb risk to gate
  // against. Any other yauzl failure (truncated EOCD, multi-disk,
  // invalid central directory signature from a shifted offset) is
  // surfaced as a `corrupt-document` ExtractionError.
  await preflightZip(ctx.buffer, ctx.filename);

  let sheets: readonly XlsxSheet[];
  try {
    sheets = await readXlsxSheets(ctx.buffer);
  } catch (error: unknown) {
    if (error instanceof XlsxEncryptedError) {
      throw new ExtractionError({
  ...
  for (const sheet of sheets) {
    ...
    if (rows.length === 0) {
      ...
      if (totalContentLength > MAX_CONTENT_BYTES) {
        throw oversizeError(ctx.filename, `output size (${MAX_CONTENT_BYTES} bytes)`);
      }
      continue;
    }
