...
const BREADCRUMB_ROUTES: readonly BreadcrumbRoute[] = [
  ...
  { pattern: ROUTES.experts, build: () => ["Experts"] },
  {
    ...
    build: (p) => ["Sessions", entitySegment(p.id), "Conclusion"],
  },
  { pattern: ROUTES.sessionExport, build: (p) => ["Sessions", entitySegment(p.id), "Export"] },
  { pattern: ROUTES.sessionDetail, build: (p) => ["Sessions", entitySegment(p.id)] },
  { pattern: ROUTES.sessions, build: () => ["Sessions"] },
  { pattern: ROUTES.chatExpert, build: (p) => ["Chats", entitySegment(p.slug)] },
  { pattern: ROUTES.chatPanel, build: (p) => ["Chats", entitySegment(p.name)] },
  { pattern: ROUTES.chats, build: () => ["Chats"] },
