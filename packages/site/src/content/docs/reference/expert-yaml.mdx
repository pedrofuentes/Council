...
|-------|------|----------|-------------|
| `slug` | string | Yes | Short identifier scoped to the panel (e.g., `cto`, `skeptic`). Lowercase, digits, hyphens; must start with a letter. |
...
- **Slug pattern**: `^[a-z][a-z0-9-]*$` (lowercase, digits, hyphens; must start with a letter).
...
- [Panel YAML Format](/Council/docs/reference/panel-yaml/)
