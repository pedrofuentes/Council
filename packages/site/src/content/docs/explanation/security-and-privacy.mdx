...
Council telemetry is **opt-in and off by default**. Enable or inspect it with
the [`council telemetry`](/Council/reference/commands/telemetry/) command:
...
When telemetry is enabled, [Council Console](/Council/how-to/use-the-terminal-ui/)
...
The file is created only after you opt in and only once a counter is recorded.
Disable telemetry to stop updating it, and delete the collected counts at any
time by removing the file:
...
- [Document RAG](/Council/explanation/document-rag) — how Council indexes and retrieves untrusted documents
- [Persona Experts](/Council/explanation/persona-experts) — how document-grounded experts handle untrusted text
- [Memory Model](/Council/explanation/memory-model) — what's stored in SQLite and how it's used
- [Architecture Overview](/Council/explanation/architecture-overview) — the engine abstraction that isolates provider details
