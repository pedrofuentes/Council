...
council expert create \
  ...
  --kind persona \
  --persona "You are a seasoned enterprise risk analyst..." \
...
Now convene a panel that includes your new expert. Use the `--expert` flag to specify custom experts by slug:
...
1. Council resolves each `--expert` slug from your library
...
- ✅ Used a custom expert in a debate with `--expert <slug>`
- ✅ Listed and managed experts in your library

## Next steps

- [Tutorial 6: Build a Custom Panel](/Council/docs/tutorials/06-build-a-custom-panel/) — combine multiple custom experts into a reusable panel template
