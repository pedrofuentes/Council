...
5. **LLM extraction needs bounded retry and `finally`-tear-down.** A single retry on malformed JSON catches transient streaming truncation without exploding cost; persistent failure throws with the raw response embedded. The transient analyzer expert MUST be removed in `finally`; cleanup failures must surface as warnings, not mask the underlying analysis result.
6. **Recency weighting is a prompt-side annotation, not a retrieval-time score (so far).** Roadmap 6.8 ships as `[Weight: 0.NN]` tags on document blocks in the analyzer meta-prompt, not as a multiplier on FTS5 BM25 scores. The LLM weights documents qualitatively — retriever ranking is unchanged. `halfLifeDays <= 0` clamps to weight 1.0 (never zeros every doc); future-dated `modifiedAt` clamps to 1.0 (never amplifies).
7. **Analyzer outputs should never be assumed JSON-clean.** Parse defensively: tolerate ```` ```json ```` fences, strip leading prose, and validate against a Zod schema before persisting. The same lesson applies to `auto-compose.ts` and `conclude.ts`.
