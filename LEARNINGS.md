...
**Learning**:
1. **`createDatabase(path)` costs ~5s per call on Windows** (11 sequential migrations + file I/O + Windows Defender real-time scanning). `createDatabase(":memory:")` costs ~12ms — a 400× difference. The suite had 119 file-backed calls vs 25 in-memory. Converting 7 unit test files to `:memory:` dropped `persister.test.ts` from 73s to 2.7s (27×) and `debate-repo.test.ts` from 24s to 71ms (340×).
...
4. **Cap `maxWorkers` on Windows.** Without a cap, Vitest defaults to CPUs-1 workers all simultaneously creating/migrating/cleaning SQLite files in `os.tmpdir()`, causing catastrophic I/O contention. `maxWorkers: "50%"` (local) and `2` (CI) eliminated the thrashing.
