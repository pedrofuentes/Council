...
- ✅ **9.5 Expert Authoring & Training** — full expert lifecycle in the TUI over a 100%-covered `expert-authoring` adapter: create generic + persona experts (guided form mirroring `ExpertDefinitionSchema`), edit (slug read-only), delete with affected-panel warnings (delete gated until the warning loads). Documents: list/remove indexed persona docs (`/experts/:slug/docs`, surfacing `ftsCleanupFailed`). Training: add a local document and refresh the profile (`/experts/:slug/train`) via the document processor under an injectable engine (MockEngine-tested offline), with a double-submit guard and `lstat`/`COPYFILE_EXCL` staging (no symlink-follow, no overwrite)
- ✅ **9.6 Panel Authoring** — author panels in the TUI over a 100%-covered `panel-authoring` adapter (create/setMembers/delete mirroring the CLI's `persistPanelArtifacts` + delete flow; debate sessions preserved): a reusable `MultiSelectList` checkbox component, a create screen (`/panels/new`), edit-members (`/panels/:name/members`), a gated delete confirm (`/panels/:name/delete`), and an engine-backed auto-compose screen (`/panels/compose`, topic → preview → confirm) that materializes inline experts then creates the panel (pinning the trusted default model, never the composer's). Shortcuts `n`/`m`/`d`/`c` on the panel screens; all forms cancel with idle-gated Esc
